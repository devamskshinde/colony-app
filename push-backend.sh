...
if ! git remote get-url colony_backend &>/dev/null; then
    info "Adding remote 'colony_backend'..."
    git remote add colony_backend https://github.com/devamskshinde/colony_backend.git
...
info "Pushing backend/ subtree to colony_backend..."
BRANCH="$(git rev-parse --abbrev-ref HEAD)"
git subtree push --prefix=backend colony_backend main 2>&1 || {
    warn "Subtree push failed. Trying force..."
    git subtree split --prefix=backend -b backend-split-tmp
    git push colony_backend backend-split-tmp:main --force
...
echo -e "  ${BOLD}colony_backend:${RESET}  https://github.com/devamskshinde/colony_backend"
